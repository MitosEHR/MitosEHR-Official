     ...
     * to apply in the format:</p><pre><code>
{
    ...
    calculateChildBoxes: function(visibleItems, targetSize) {
        ...
        for (i = 0; i < visibleCount; i++) {
            ...
            if (me.align == 'stretch') {
                ...
                calcs[perpendicularPrefix] = constrain(stretchSize, child[perpendicularMinString] || 0, child[perpendicularMaxString] || infinity);
                calcs.dirtySize = true;
            }
    ...
    renderItem: function(item, target) {
        ...
        // String or number specification specifies all margins. No defaulting is possible.
        if (Ext.isString(margins) || Ext.isNumber(margins)) {
            margins = Ext.util.Format.parseBox(margins);
        }
        // Apply current CSS margins into any values missing from an object-based margin spec.
        else {
        ...
        // Set the element's CSS margins to the calculated values.
        itemEl.setStyle({ margin : Ext.core.Element.unitizeBox(margins) });
    },
