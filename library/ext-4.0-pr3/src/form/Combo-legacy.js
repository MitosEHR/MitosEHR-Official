 ...
 * <p>If using a ComboBox in an {@link Ext.grid.EditorGridPanel Editor Grid} a {@link Ext.grid.Header#renderer renderer}
 ...
 * @xtype combo
 */
Ext.define('Ext.form.ComboBox', {extend:'Ext.form.Trigger',
    /**
     * @cfg {Mixed} transform The id, DOM node or element of an existing HTML SELECT to convert to a ComboBox.
     * Note that if you specify this and the combo is going to be in an {@link Ext.form.Basic} or
     ...
     */
    /**
     * @cfg {String/Object} autoCreate <p>A {@link Ext.core.DomHelper DomHelper} element spec, or <tt>true</tt> for a default
    ...
    /**
     * @cfg {Number} listWidth The width (used as a parameter to {@link Ext.core.Element#setWidth}) of the dropdown
     ...
     */
    selectedClass: Ext.baseCSSPrefix + 'combo-selected',
    /**
     ...
     */
    triggerClass: Ext.baseCSSPrefix + 'form-arrow-trigger',
    /**
    ...
    shadow: 'sides',
    /**
     * @cfg {String/Array} listAlign A valid anchor position value. See <tt>{@link Ext.core.Element#alignTo}</tt> for details
     * on supported anchor positions and offsets. To specify x/y offsets as well, this value
     * may be specified as an Array of <tt>{@link Ext.core.Element#alignTo}</tt> method arguments.</p>
     ...
     * <tt>{@link Ext.form.Trigger#editable editable} = false</tt>).
     */
    minChars: 4,
    /**
     ...
     * {@link Ext.toolbar.PagingToolbar#pageSize limit} parameters. Only applies when <tt>{@link #mode} = 'remote'</tt>
     * (defaults to <tt>0</tt>).
     */
    pageSize: 0,
    /**
     * @cfg {Boolean} selectOnFocus <tt>true</tt> to select any existing text in the field immediately on focus.
     * Only applies when <tt>{@link Ext.form.Trigger#editable editable} = true</tt> (defaults to
     * <tt>false</tt>).
     */
    selectOnFocus: false,
    /**
...
var combo = new Ext.form.ComboBox({
    ...
    initComponent: function() {
        ...
        if (this.transform) {
            ...
            if (!this.lazyRender) {
                this.target = true;
                this.el = Ext.core.DomHelper.insertBefore(s, this.autoCreate || this.defaultAutoCreate);
    ...
    getParentZIndex: function() {
        ...
        if (this.ownerCt) {
            this.findParentBy(function(ct) {
                zindex = parseInt(ct.el.getStyle('z-index'), 10);
                return !! zindex;
            });
        }
        return zindex;
    ...
    initList: function() {
        if (!this.list) {
            var cls = Ext.baseCSSPrefix + 'combo-list',
            ...
            var lw = this.listWidth || Math.max(this.formEl.getWidth(), this.minListWidth);
            this.list.setSize(lw, 0);
            this.list.swallowEvent('mousewheel');
            this.assetHeight = 0;
            if (this.syncFont !== false) {
                this.list.setStyle('font-size', this.inputEl.getStyle('font-size'));
            }
            if (this.title) {
            ...
            if (this.pageSize) {
                ...
                this.pageTb = new Ext.toolbar.PagingToolbar({
                    store: this.store,
...
var menu = new Ext.menu.Menu({
    ...
    bindStore: function(store, initial) {
        ...
        if (store) {
            ...
            this.store = Ext.data.StoreMgr.lookup(store);
            this.store.on({
                scope: this,
    ...
    initEvents: function() {
         ...
         * <p>A {@link Ext.util.KeyNav KeyNav} object which handles navigation keys for this ComboBox. This performs actions
         ...
         * or add your own based upon key names as specified in the {@link Ext.util.KeyNav KeyNav} class.</p>
...
myCombo.keyNav.tab = function() {   // Override TAB handling function
    this.onViewClick(false);        // Select the currently highlighted row
         ...
         */
        this.keyNav = new Ext.util.KeyNav(this.inputEl, {
            "up": function(e) {
                this.inKeyMode = true;
                this.selectPrev();
            },
            ...
            doRelay: function(e, h, hname) {
                if (hname == 'down' || this.scope.isExpanded()) {
                    ...
                    var relay = Ext.util.KeyNav.prototype.doRelay.apply(this, arguments);
...
*/
    ...
    assertValue: function() {
        ...
        if (!rec && this.forceSelection) {
            if (val.length > 0 && val != this.emptyText) {
                this.inputEl.dom.value = Ext.value(this.lastSelectionText, '');
                this.applyEmptyText();
            } else {
                this.clearValue();
            }
    ...
    restrictHeight: function() {
        ...
        this.list.alignTo.apply(this.list, [this.inputEl].concat(this.listAlign));
        this.list.endUpdate();
    },
    ...
    collapseIf: function(e) {
        if (!this.isDestroyed && !e.within(this.formEl) && !e.within(this.list)) {
            this.collapse();
        }
    },
     ...
     */
    // private
    // Implements the default empty Trigger.onTriggerClick function
