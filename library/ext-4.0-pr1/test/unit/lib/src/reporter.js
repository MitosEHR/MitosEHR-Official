...
jasmine.browser.isIE8 = jasmine.browser.isIE && !!window.XMLHttpRequest && !!document.documentMode;
jasmine.browser.isIE9 = jasmine.browser.isIE && !jasmine.browser.isIE6 && !jasmine.browser.isIE7 && !jasmine.browser.isIE8;
...
Test.SandBoxImpl = function(){
    this.sandboxes = {};
    this.iframeHTML = '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"\
"http://www.w3.org/TR/html4/loose.dtd">\
<html>\
<head>\
{head}\
</head>\
<body onload="parent.Test.SandBox.onLoad();">\
</body>\
</html>';
};
...
Test.SandBoxImpl.prototype.setup = function(config) {
    var me = this,
        iframe = document.body.appendChild(new jasmine.Dom({
            tag: "iframe",
            cls: "sandboxIframe",
            src: '',
            name: "domSandbox",
    ...
    for (i = 0; i < length; i++) {
        include = includes[i];
        if (include.type === "js") {
            head += '<script type="text/javascript" src="' + include.src + '"></script>\n';
        } else {
            head += '<link rel="stylesheet" type="text/css" href="'+ include.src + '">\n';
        }
    }
    me.iframeHTML = me.iframeHTML.replace("\{head\}", head);
...
Test.SandBoxImpl.prototype.save = function(spec) {
    var sb = this.getDoc().createElement("div"),
        children = this.getBody().childNodes,
        length = children.length,
        i = 0,
        child;
    ...
    for (; i < length; i++) {
        ...
        if (child && child.id !== "textmetrics") {
            sb.appendChild(child);
        }
    }

    this.sandboxes[spec.id] = sb;
};

Test.SandBoxImpl.prototype.onLoad = function() {
    this.getWin().eval("jasmine.getEnv().addReporter(parent.Test.SandBox.reporter); jasmine.getEnv().execute();");
};

Test.SandBoxImpl.prototype.isRunning = function() {
    return this.getWin().eval("!jasmine.getEnv().currentRunner_.queue.isRunning();");
};
...
Test.BadGlobalsImpl = function(reporter) {
    ...
    this.allowedGlobals = [];
    for (property in win) {
        if (jasmine.browser.isOpera && win[property] && win[property].tagName) {
            continue;
        }
 ...
 * @return {Boolean} 
 */
Test.BadGlobalsImpl.prototype.check = function() {
...
Test.BadGlobalsImpl.prototype.report = function(info, suite) {
    ...
    for (property in win) {
        ...
        if (jasmine.array.indexOf(allowedGlobals, property) === -1) {
            info = info;
            info.log("Bad global variable found in suite " + suite.description + "<br/>" + property + " = " + jasmine.pp(win[property]), "warning");        
    ...
    add: function(suite) {
        ...
        if (coverage && file) {
            for (property in file) {
                ...
                if (statement > 1 && property !== "source") {
                    file[property] = 0;
                }
            }
        }  
 ...
 * @param {jasmine.Suite} The jasmine suite.
 */
    updateSuiteEl: function(suiteEl, suite) {
        var description;
        if (suite.jscoverage) {
            description = suiteEl.childNodes[1];
            jasmine.Dom.setHTML(description, description.innerHTML + " - Code coverage: " + suite.jscoverage.percentage + "%");
        }
    },
...
Test.panel.Sandbox.prototype.afterRender = function() {
    var html, doc, iframe, win;

    html = '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"\
"http://www.w3.org/TR/html4/loose.dtd">\
<html><head>' + Test.SandBox.getHead().innerHTML + "</head><body>" + Test.SandBox.getBody().innerHTML + "</body></html>";

    iframe = this.el.childNodes[0];
...
Test.panel.Sandbox.prototype.render = function() {
    var iframeBody = Test.SandBox.getBody(),
        children = this.sandBox.childNodes,
        length = children.length,
        i = 0,
        child, html;
    ...
    Test.SandBox.onDomLoad = function() {
...
Test.panel.Sandbox.prototype.remove = function() {
    var iframe = Test.SandBox.getIframe(),
        children = Test.SandBox.getBody().childNodes,
        length = children.length,
        i = 0,
        child;
    ...
    for (; i < length; i++) {
        child = children[i];
        this.sandBox.appendChild(child);
    }
...
Test.panel.Infos.prototype.remove = function() {
    this.el.parentNode.removeChild(this.el);
};
/**
...
Test.panel.jsCoverageSummary.prototype.remove = function() {
    this.el.parentNode.removeChild(this.el);
};
/**
...
Test.panel.TabPanel.prototype.activatePanel = function(cls) {
    ...
    if (this.spec) {
        ...
        if (cls === "domSandbox") {
            this.add(new Test.panel.Sandbox({
                spec: this.spec,
...
Test.panel.TabPanel.prototype.renderToolBar = function() {
    ...
    if (spec) {
        sandbox = this.sandboxes[spec.id];
        ...
        if (sandbox) {
            this.addTab("domSandbox", "Sandbox");
        }
    }
...
Test.panel.TreeGrid.prototype.addSuite = function(suite) {
    ...
    this.suitesEls[suite.id] = new jasmine.Dom({
        tag: "div",
        id: "suite-" + suite.id,
        cls: "suite " + (!suite.parentSuite ? "root " : ""),
        style: {
        ...
        children: [{
            ...
            tag: "span",
            cls: "description",
            html: "describe: " + suite.description
        }]
    });
 ...
 */
Test.panel.TreeGrid.prototype.filterSpec = function(spec) {
    ...
    if (suites) {
        suites = jasmine.isArray_(suites) ? suites : [suites];
        length = suites.length;
        spec.skipped = true;
        while(suite) {
            for (i = 0; i < length; i++) {
                if (suite.id == suites[i]) {
                    delete spec.skipped;
                    break;
                }
            }
    ...
    if (specs) {
        specs = jasmine.isArray_(spec) ? specs : [specs];
        length = specs.length;
        if (!suites) {
            spec.skipped = true;
        }
...
Test.panel.TreeGrid.prototype.onCheck = function(el) {
    ...
    if (jasmine.Dom.hasCls(next,"spec")) {
        id = next.id.replace("spec-", "");
        this.options.specs = this.options.specs || [];
        if (!jasmine.isArray_(this.options.specs)) {
            this.options.specs = [this.options.specs];
        }
        if (el.checked) {
            ...
            if (jasmine.isArray_(this.options.specs)) {
               ...
               delete this.options.specs;
            }
        }
    } else {
        id = next.id.replace("suite-", "");
        this.options.suites = this.options.suites || [];
        if (!jasmine.isArray_(this.options.suites)) {
            this.options.suites = [this.options.suites];
        }
        if (el.checked) {
...
Test.panel.TreeGrid.prototype.onCollapse = function(el) {
    ...
    if (jasmine.Dom.hasCls(el, "suite")) {
        this.onEachRow(el.parentNode, function(row, o) {
            ...
            jasmine.Dom.setCls(row.childNodes[1].childNodes[0], "expand");
            if (o.suite) {
                row.childNodes[1].childNodes[2].style.display = "none";
            }
        });
    } else {
...
Test.Reporter.prototype.reportRunnerStarting = function(runner) {
    ...
    if (!window.headless) {
        this.treeGrid = new Test.panel.TreeGrid({
            sandboxes: Test.SandBox.sandboxes
        });
        
        this.treeGrid.log("Started at " + this.startedAt.toString());
        
        if (!this.options.remote) {
            this.treeGrid.log("Warning! Because you access TestReporter locally, stack trace report isn't available.");
        }
...
Test.Reporter.prototype.reportRunnerResults = function(runner) {
    ...
    if(window.headless && window.headless.onRunnerEnd) {
        window.headless.onRunnerEnd(this, runner);
    }
};
...
Test.Reporter.prototype.reportSuiteResults = function(suite) {
    var status = suite.results().passed() ? "passed" : "failed",
        suiteEl = this.treeGrid ? this.treeGrid.suitesEls[suite.id] : undefined;
        
    Test.jsCoverage.update(suite);
  
    
    if (!jasmine.browser.isIE && !suite.parentSuite && Test.BadGlobals.check()){
        Test.BadGlobals.report(window.headless || this.treeGrid || console, suite);
    }
    
    if (suiteEl) {
        jasmine.Dom.addCls(suiteEl, status);
        Test.jsCoverage.updateSuiteEl(suiteEl, suite);
    }

    if(window.headless && window.headless.onSuiteEnd) {
        window.headless.onSuiteEnd(this, suite);
    }
};
...
Test.Reporter.prototype.reportSpecResults = function(spec) {
    ...
    if (!spec.skipped) {
        ...
        if ((status === "failed" || this.options.showPassed) && !spec.skipped && !window.headless) {
            this.treeGrid.addSpec(spec);
        }

        Test.SandBox.save(spec);
        
        if(window.headless && window.headless.onSpecEnd) {
            window.headless.onSpecEnd(this, spec);
        }
    }
};
...
Test.Reporter.prototype.renderResults = function(runner) {
    ...
    this.treeGrid.log("Finished at " + new Date().toString());
};
 ...
 * @param {jasmine.Spec} spec The spec to filter.
 */
Test.Reporter.prototype.filterSpec = function(spec) {
    var suite = spec.suite,
        suites = this.options.suites,
        specs = this.options.specs, 
        length, i;
