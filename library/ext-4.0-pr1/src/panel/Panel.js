...
Ext.define('Ext.panel.Panel', {
     ...
     * be used as the default value for the <tt>{@link Ext.form.Button#minWidth}</tt> config of
     * each Button added to the <b>footer toolbar</b>. Will be ignored for buttons that have this value configured some
     * other way, e.g. in their own config object or via the {@link Ext.lib.Container#defaults defaults config} of
     ...
     * containing a {@link Ext.panel.Panel.Tool Tool} to re-expand the Panel.</p>
     */
    ...
    collapse: function(direction, animate) {
        ...
        // Track perpendicular docked items which he hide during collapsed state
        me.hiddenDocked = [];
        ...
        if (me.collapsed || me.fireEvent('beforecollapse', me, direction, animate, newSize) === false) {
            return false;
        }
    ...
    expand: function(animate) {
        ...
        if (animate) {
            var satisfyJSLint = new Ext.fx.Anim(anim);
        } else {
            me.setSize(anim.to.width, anim.to.height);
            if (anim.to.x) {
                me.setLeft(anim.to.x);
    ...
    toggleCollapse: function() {
        this.collapsed ? this.expand(this.animCollapse) : this.collapse(this.collapseDirection, this.animCollapse);
        return this;
    },
