 ...
 * an initial <code>width</code>, or a {@link Ext.layout.container.Box#flex} value. The 'center' region has a flex value of <code>1</code>.</li>
 * <li>Any child items with a region of <code>north</code> or <code>south</code> may be configured with either
 * an initial <code>height</code>, or a {@link Ext.layout.container.Box#flex} value. The 'center' region has a flex value of <code>1</code>.</li>
 ...
 * </code></pre>
 * </li>
 * <li><b>There is no Region class in ExtJS 4.0+</b></li>
 * </ul></div>
 */
Ext.define('Ext.layout.container.Border', {
    ...
    initializeBorderLayout: function() {
        ...
        for (; i < ln; i++) {
            ...
            if (comp.collapsible && comp.collapseMode != 'header' && comp.collapseMode != 'mini') {
                comp.on({
...
//      Begin the VBox and HBox item list.
        ...
        if (regions.north || regions.south) {
            ...
            // Inject items into the HBox Container if there is one.
    ...
    onBeforeRegionCollapse: function(comp, direction, animate, newSize) {
        ...
        compBox = shadowLayout.getChildBox(comp);
        ...
        function afterCollapse() {
            ...
            // Defer it so  that it happens outside of the animation's event.
        ...
        switch (comp.region) {
            case 'north':
                placeHolderBox.top = compBox.top + compBox.height - placeHolderBox.height;
                compAnim.to.top = -compBox.height;
                break;
            case 'south':
                placeHolderBox.top = compBox.top;
                compAnim.to.top = targetSize.height;
                break;
            case 'east':
                placeHolderBox.left = compBox.left;
                compAnim.to.left = targetSize.width;
                break;
            case 'west':
                placeHolderBox.left = compBox.left + compBox.width - placeHolderBox.width;
                compAnim.to.left = -compBox.width;
        }
        ...
        // Set everything to the new positions.
        if (comp.animCollapse) {
            ...
            var satisfyJSLint = new Ext.fx.Anim(compAnim);
        } else {
            shadowLayout.layout();
            afterCollapse();
        }
    ...
    getCollapsedPosition: function(comp) {
        ...
        result.top = comp.region == 'north' ? -comp.height : comp.region == 'south' ? targetSize.height : shadowLayout.padding.top;
        result.left = comp.region == 'west' ? -comp.width : comp.region == 'east' ? targetSize.width : shadowLayout.padding.left;
        return result;
    },
    ...
    onPlaceHolderToolClick: function(e, target, owner, tool) {
        ...
        // If the Component is already floated when they click the placeHolder Tool,
        ...
        if (comp.slideOutAnim) {
            ...
            if (comp.slideInAnim && comp.slideInAnim.running) {
                comp.slideInAnim.end();
                delete comp.slideInAnim;
            }
        ...
        // Equalize the perpendicular measurement of the expanding Component prior to animation
        // in case the layout area has changed size during the time it was collapsed.
        if (fromCompBox[perpendicularPrefix] != toCompBox[perpendicularPrefix]) {
            comp['set' + shadowLayout.perpendicularPrefixCap].call(comp, toCompBox[perpendicularPrefix]);
            delete comp[perpendicularPrefix];
        }
    ...
    floatCollapsedPanel: function(e, comp) {
        var me = this,
            ...
            shadowContainer = comp.shadowOwnerCt,
            shadowLayout = shadowContainer.layout,
            compHiddenBox = me.getCollapsedPosition(comp),
        ...
        // If the Component is already floated when they click the placeHolder,
        // it will be primed with a slide out animation object... so slide it out
        // and remove the mouseout listeners
        if (comp.slideOutAnim) {
            // It's *being* slid out: Stop that, we'll slide it back in.
            if (comp.slideOutAnim.running) {
                comp.slideOutAnim.end();
            }
            // It's *ready* to be slid out: Do it now.
            else {
                me.slideOutFloatedComponent(comp);
                return;
            }
        ...
        compBox = shadowLayout.calculateChildBox(comp);

        switch (comp.region) {
            case 'north':
                compBox.top += placeHolderBox.height;
                break;
            case 'south':
                compBox.top -= placeHolderBox.height;
                break;
            case 'east':
                compBox.left -= placeHolderBox.width;
                break;
            case 'west':
                compBox.left += placeHolderBox.width;
        }

        // This animation slides the collapsed Component out to just beyond its placeHolder
        compAnim = {
            target: comp,
            from: compHiddenBox,
            to: {
        ...
        // Equalize the perpendicular measurement of the expanding Component prior to animation
        // in case the layout area has changed size during the time it was collapsed.
        if (compAnim.from[perpendicularPrefix] != compAnim.to[perpendicularPrefix]) {
            comp['set' + shadowLayout.perpendicularPrefixCap].call(comp, compAnim.to[perpendicularPrefix]);
            delete comp[perpendicularPrefix];
            compAnim.from[perpendicularPrefix] = compAnim.to[perpendicularPrefix];
        }
    ...
    slideOutFloatedComponent: function(comp) {
        ...
        slideOutAnim = new Ext.fx.Anim(comp.slideOutAnim);
