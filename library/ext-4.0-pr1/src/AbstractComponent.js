...
renderSelectors: {
     ...
     * <p>The {@link Ext.layout.AutoComponentLayout default layout manager} will be used on instances of the base Ext.Component class
   ...
   /**
    ...
    onRender : function(container, position) {
        ...
        if (!el) {
            if (position) {
                el = Ext.core.DomHelper.insertBefore(position, this.getElConfig(), true);
            }
            else {
                el = Ext.core.DomHelper.append(container, this.getElConfig(), true);
            }
        }
        ...
        if (renderTpl) {
            ...
            renderTpl.append(this.getTargetEl(), renderData);
        }
    ...
    initFrame : function() {
        var frameBaseCls = this.baseCls + (this.ui ? '-' + this.ui : ''),
            info = this.el.getStyle('background-image'),
            frameWidth = 0, frameSize,
            frameRenderTarget, frameTpl,
            isTable, radius;

        // We actually pass a string in the form of 'type-internal-radius-framesize-tl-tr-br-bl' as
        // the background image of this.el from the css to indicate to IE that this component needs
        // framing. We parse it here and change the markup accordingly.
        if (info && info.indexOf('internal-radius-') != -1) {
            if (info.indexOf('table') != -1) {
                ...
                this.el.addCls(this.initCls());
                this.el.setStyle(this.initStyles());

                // The true makes sure we get the table framing
                frameTpl = this.getFrameTpl(true);

                // The render target for the frame now becomes the tbody instead of this.getTargetEl
                frameRenderTarget = this.el.down('tbody');
            }
            else {
                frameTpl = this.getFrameTpl();
                frameRenderTarget = this.getTargetEl();
            }

            // Get and parse the different border radius sizes
            radius     = info.match(/internal\-radius\-([0-9]{1,2})\-([0-9]{1,2})\-([0-9]{1,2})\-([0-9]{1,2})\-([0-9]{1,2})/);
            frameWidth = parseInt(radius[1], 10);
            frameSize  = {
                top:    Math.max(radius[2], radius[3]),
                right:  Math.max(radius[3], radius[4]),
                bottom: Math.max(radius[4], radius[5]),
                left:   Math.max(radius[5], radius[2])
            };
        ...
        if (this.frame === true && !radius) {
            //<debug error>
            throw new Error("[" + Ext.getClassName(this) + "#initFrame] You have set frame: true explicity on this component " +
        ...
        this.frame = this.frame || !!frameWidth;
        this.frameSize = frameSize || false;

        if (this.frame) {
            //<debug error>
            if (!frameSize) {
                throw new Error("[" + Ext.getClassName(this) + "#initFrame] Unable to read background-image style " +
                                "(got '" + info + "') of element: " + this.el.dom.outerHTML + " to handle framing.");
            }
            //</debug>
            ...
            this.frameBody = this.el.down('.' + frameBaseCls + '-mc');
        }
    },
    ...
    initCls: function() {
        ...
        if (this.frame) {
            cls.push('x-framed ' + this.baseCls + '-' + (this.ui ? this.ui + '-' : '') + 'framed');
        }
        ...
        if (Ext.isString(this.style)) {
            ...
            for (i = 0, ln = split.length; i < ln; i++) {
                if (!Ext.isEmpty(split[i])) {
                    prop = split[i].split(':');
                    style[Ext.String.trim(prop[0])] = Ext.String.trim(prop[1]);
                }
            }
        } else {
    ...
    initContent: function() {
        ...
        if (this.html) {
            target.update(Ext.core.DomHelper.markup(this.html));
            delete this.html;
        }
...
alert(t.getXTypes());  // alerts 'component/field/textfield'
    ...
    addListener : function(element, listeners, scope, options) {
        ...
        return this.mixins.observable.addListener.apply(this, arguments);
    },
    ...
    setSize : function(width, height) {
        ...
        if (Ext.isNumber(width)) {
            width = Ext.Number.constrain(width, this.minWidth, this.maxWidth);
        }
        if (Ext.isNumber(height)) {
            height = Ext.Number.constrain(height, this.minHeight, this.maxHeight);
        }
        ...
        if (!this.rendered || !this.isVisible()) {
            ...
            if (this.hiddenAncestor) {
                var layoutCollection = this.hiddenAncestor.layoutOnShow;
    ...
    setCalculatedSize : function(width, height, ownerCt) {
        ...
        this.doComponentLayout(width, height, false, ownerCt);
    ...
    doComponentLayout : function(width, height, isSetSize, ownerCt) {
        ...
        // docked items parallel to the collapseTo side are visible, and the Panel body is hidden.
    ...
    getComponentLayout : function() {
        if (!this.componentLayout || !this.componentLayout.isLayout) {
            this.setComponentLayout(Ext.layout.Manager.create(this.componentLayout, 'autocomponent'));
        }
     ...
     */
    getLoader: function(){
        var autoLoad = this.autoLoad ? (Ext.isObject(this.autoLoad) ? this.autoLoad : {url: this.autoLoad}) : null,
            loader = this.loader || autoLoad;

        if (loader) {
            if (!loader.isLoader) {
                this.loader = Ext.create('Ext.ComponentLoader', Ext.apply({
                    target: this,
                    autoLoad: autoLoad
                }, loader));
            } else {
