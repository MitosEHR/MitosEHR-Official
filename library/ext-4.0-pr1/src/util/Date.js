...
dt = Ext.Date.parseDate("2006-02-29 03:20:01", "Y-m-d H:i:s", true); // returns null
    ...
    createFormat : function(format) {
        ...
        Ext.util.Date.formatFunctions[format] = new Function("return " + code.join('+'));
    },
    ...
    createParser : (function() {
        ...
        return function(format) {
            ...
            Ext.util.Date.parseFunctions[format] = new Function("input", "strict", xf(code, regexNum, calc.join('')));
        };
    })(),
    ...
    getGMTOffset : function(date, colon) {
        return (Ext.util.Date.getTimezoneOffset(date) > 0 ? "-" : "+")
            + Ext.String.leftPad(Math.floor(Math.abs(date.getTimezoneOffset()) / 60), 2, "0")
            + (colon ? ":" : "")
            + Ext.String.leftPad(Math.abs(date.getTimezoneOffset() % 60), 2, "0");
    },
    ...
    getWeekOfYear : (function() {
        ...
        return function(date) { // return a closure so constants get calculated only once
            var DC3 = Ext.Date.UTC(date.getFullYear(), date.getMonth(), date.getDate() + 3) / ms1d, // an Absolute Day Number
                AWN = Math.floor(DC3 / 7), // an Absolute Week Number
                Wyr = new Date(AWN * ms7d).getUTCFullYear();

            return AWN - Math.floor(Ext.Date.UTC(Wyr, 0, 7) / ms7d) + 1;
        };
    })(),
