...
var applyLinkCallbacks = function() {
    ...
    var memberOverStates = {
        ...
        'subClasses': { but: false, tab: false },
        'mixins': { but: false, tab: false },
        'mixedInto': { but: false, tab: false },
        'superClasses': { but: false, tab: false }
    }
        ...
        Ext.MessageBox.confirm('Confirm', 'Are you sure you want to do that?', function(btn) {
            if (btn == 'yes') {
                Ext.Ajax.request({
                    ...
                    method  : 'POST',
                    url     : req.baseDocURL + '/api/' + req.docClass + '/examples/' + el.rel + '/delete',

                    success : function(response, opts) {
                        Ext.get('exampleList').update(response.responseText);
                        applyLinkCallbacks();
                    },
                    failure : function(response, opts) {
    ...
    function bindTabs(tabId) {
      ...
      if(t) {
        t.on('click', function(e, el) {
          ...
          var tab = Ext.get('doc-'+tabId);
          if (tab) tab.setStyle({display: 'block'});
        });
      }
    }
    ...
    var expandAllMembers = Ext.get('expandAllMembers');
    if (expandAllMembers) {
    ...
    var collapseAllMembers = Ext.get('collapseAllMembers');
...
Ext.onReady(function() {
  ...
  var huc = Ext.get('hideUnpopularClasses');
  if(huc) {
      huc.on('click', function(e, el) {
          Ext.query('.unpopular').forEach(function(m) {
              if(el.checked) {
                  Ext.get(m).setStyle({display: 'none'});
              } else {
                  Ext.get(m).setStyle({display: 'block'});
