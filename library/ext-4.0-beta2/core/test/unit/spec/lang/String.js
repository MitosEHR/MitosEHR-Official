describe("Ext.String", function() {
    ...
    describe("parseQueryString", function(){
        ...
        var parseQueryString = Ext.String.parseQueryString;
        
        it("should return an empty object for a null value", function(){
        ...
        it("should handle a single parameter", function(){
            expect(parseQueryString('foo=bar')).toEqual({
                foo: 'bar'
            });
        });
        
        it("should handle a parameter with any value", function(){
        ...
        it("should handle multiple parameters", function(){
            expect(parseQueryString('param1=a&param2=b&param3=c')).toEqual({
                param1: 'a',
                param2: 'b',
                param3: 'c'
            });
        ...
        it("should always return string values", function(){
            var result = parseQueryString('foo=1&bar=false');
            expect(typeof result.foo).toEqual('string');
            expect(typeof result.bar).toEqual('string');
        });
        
        it("should not overwrite params by default", function(){
            expect(parseQueryString('foo=1&foo=2&foo=3')).toEqual({
                foo: ['1', '2', '3']
            });
        });
        
        it("should ignore the overwrite param if all the keys are unique", function(){
            expect(parseQueryString('foo=val1&bar=val2', true)).toEqual({
                foo: 'val1',
                bar: 'val2'
            });
        });
        
        it("should take the last param if overwrite is set to true", function(){
            expect(parseQueryString('foo=1&foo=2&foo=3', true)).toEqual({
                foo: '3'
            });
        });
        
        it("should overwrite with multiple of the same params", function(){
            expect(parseQueryString('foo=1&foo=2&foo=3&bar=4&bar=5&bar=6', true)).toEqual({
                foo: '3',
                bar: '6'
            });
        });
